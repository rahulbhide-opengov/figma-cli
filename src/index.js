...
create
  .command('image-nobg <url>')
  .description('Create an image with background removed (uses rembg, free & local)')
  .option('-w, --width <n>', 'Width (auto if not set)')
  .option('-h, --height <n>', 'Height (auto if not set)')
  .option('-x <n>', 'X position (auto if not set)')
  .option('-y <n>', 'Y position', '0')
  ...
  .action(async (url, options) => {
    ...
    // Find rembg binary (check common locations)
    const rembgPaths = [
      'rembg',
    ...
    if (!rembgBin) {
      console.log(chalk.red('✗ rembg not installed'));
      console.log(chalk.gray('\nInstall it with:'));
      console.log(chalk.cyan('  pipx install "rembg[cli]"'));
      console.log(chalk.gray('\nThen try again. rembg is free and runs locally.'));
      return;
    }
    ...
    try {
      ...
      const tempInput = '/tmp/figma-cli-input.png';
      const tempOutput = '/tmp/figma-cli-nobg.png';
      ...
      spinner.text = 'Removing background (this may take a moment)...';

      // Run rembg
      execSync(`"${rembgBin}" i "${tempInput}" "${tempOutput}"`, { stdio: 'ignore' });
      ...
      spinner.text = 'Creating image in Figma...';
...
`;
      ...
      const result = figmaUse(`eval "${code.replace(/"/g, '\\"').replace(/\n/g, ' ')}"`, { silent: true });
      spinner.succeed('Background removed and image created');
      if (result) console.log(chalk.gray(result.trim()));
